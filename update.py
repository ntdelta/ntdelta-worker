...
import subprocess
import pefile
from retrieve_pdb import get_pe_debug_infos, download_pdb
import argparse
...
WINBINDEX_INSIDER_BASE = "https://m417z.com/winbindex-data-insider/by_filename_compressed/{}.json.gz"
WORKING_DIR_PATH = "C:\\Users\\Administrator\\ntdelta\\working_dir\\"
SYMPATH = "C:\\Symbols\\"
GHIDRA_HEADLESS = "C:\\Users\\Administrator\\Desktop\\ghidra_10.1.5_PUBLIC_20220726\\ghidra_10.1.5_PUBLIC\\support\\analyzeHeadless.bat"
ADDITIONAL_DLLS = []
...
def get_dll_exports(input_dll_path):
    ...
    try:
        ...
        exports = [decode_if_not_none(e.name) for e in pe.DIRECTORY_ENTRY_EXPORT.symbols]
        return exports
...
def get_all_files(directory, file_set=None):
    ...
    for filename in os.listdir(directory):
        ...
        if os.path.isfile(path):
            ...
            elif not filename[:70] in file_set.keys():
...
def make_symbol_server_url(pe_name, time_stamp, image_size):
    ...
    file_id = ('%s%X' % (formatted_timestamp, image_size)).lower()
...
def get_winbindex_json(tracked_dll_name, insider=False):
    ...
    if insider:
        tracked_dll_winbindex_compressed_url = WINBINDEX_INSIDER_BASE.format(tracked_dll_name)

    tracked_dll_winbindex_compressed_path = WORKING_DIR_PATH + "{}.json.gz".format(tracked_dll_name)
    tracked_dll_winbindex_path = WORKING_DIR_PATH + "{}.txt".format(tracked_dll_name)

    # Get compressed winbindex info
    tracked_dll_winbindex_compressed_response = requests.get(tracked_dll_winbindex_compressed_url, stream=True)
...
if tracked_dlls_response.status_code == 200:
    ...
    for tracked_dll_name in list(set(list(tracked_dlls.keys())+ADDITIONAL_DLLS)):
        ...
        if tracked_dll_name in INSIDER_DLLS:
            tracked_dll_winbindex_insider_json_data = get_winbindex_json(tracked_dll_name, True)
            if tracked_dll_winbindex_insider_json_data:
                tracked_dll_winbindex_json_data = tracked_dll_winbindex_json_data | tracked_dll_winbindex_insider_json_data
        ...
        if tracked_dll_winbindex_json_data:
            ...
            tracked_dll_versions_response = requests.get(BASE_URL + "dlls/" + tracked_dll_name)
            if new_dll_flag or tracked_dll_versions_response.status_code == 200:
                tracked_dll_versions = set()
                if not new_dll_flag:
                    tracked_dll_versions = set([i['sha256'] for i in tracked_dll_versions_response.json()['instances']])
                tracked_dll_winbindex_versions = set(tracked_dll_winbindex_json_data.keys())
                missing_versions = tracked_dll_winbindex_versions - tracked_dll_versions
                for missing_version_hash in missing_versions:
                    missing_version = tracked_dll_winbindex_json_data[missing_version_hash]

                    # Check DLL architecture. We only care about x64
                    if "wow64" in str(missing_version):
                        continue

                    if "fileInfo" in missing_version.keys() and "version" in missing_version['fileInfo'].keys():
                        # We don't care about forwarder shims... (OneCore)
                        if "forwarder shim" in missing_version['fileInfo']['description']:
                            continue
                        
                        logging.info("Missing DLL found")

                        tracked_dll_version_file_path = WORKING_DIR_PATH + tracked_dll_name + os.sep + missing_version_hash + ".blob"

                        tracked_dll_version_ts = missing_version['fileInfo']['timestamp']
                        tracked_dll_version_size = missing_version['fileInfo']['virtualSize']

                        tracked_dll_version_download_pe_url = make_symbol_server_url(tracked_dll_name,
                                                                                     tracked_dll_version_ts,
                                                                                     tracked_dll_version_size)

                        logging.info("Downloading from: " + tracked_dll_version_download_pe_url)

                        tracked_dll_version_pe_download = requests.get(tracked_dll_version_download_pe_url, stream=True)
                        if tracked_dll_version_pe_download.status_code == 200:
                            ...
                            pdb_name, guid = get_pe_debug_infos(tracked_dll_version_file_path)
                            if not os.path.exists(SYMPATH + pdb_name + os.sep + guid + os.sep + pdb_name):
                                download_pdb(WORKING_DIR_PATH + tracked_dll_name, pdb_name, guid)
                                tracked_dll_version_local_pdb_path = WORKING_DIR_PATH + tracked_dll_name + os.sep + guid
                                tracked_dll_version_sympath_pdb_dir = "{}{}{}{}".format(SYMPATH, pdb_name, os.sep, guid)
                                if not os.path.exists(tracked_dll_version_sympath_pdb_dir):
                                    os.makedirs(tracked_dll_version_sympath_pdb_dir)
                                shutil.copy(tracked_dll_version_local_pdb_path,
                                            "{}{}{}{}{}{}".format(SYMPATH, pdb_name, os.sep, guid, os.sep, pdb_name))
                                remove_file_if_exists(tracked_dll_version_local_pdb_path)

                            logging.info("Symbols all set up. Time to decompile.")

                            # Ghidra time
                            tracked_dll_proj_name = "ghidra" + missing_version_hash
                            tracked_dll_proj_folder = WORKING_DIR_PATH + tracked_dll_proj_name
                            tracked_dll_proj_output_json = WORKING_DIR_PATH + tracked_dll_proj_name + os.sep + "decompile.json"
                            tracked_dll_proj_decomp_output_path_format = WORKING_DIR_PATH + tracked_dll_proj_name + os.sep + tracked_dll_proj_name + "_"
                            tracked_dll_proj_decomp_output_path_format = tracked_dll_proj_decomp_output_path_format.replace(
                            ...
                            if os.path.exists(tracked_dll_proj_folder):
                                ...
                                print(f"Folder '{tracked_dll_proj_folder}' deleted successfully.")

                            # # Create folder
                            if not os.path.exists(tracked_dll_proj_folder):
                                os.makedirs(tracked_dll_proj_folder)

                            copy_file("DumpFunctions.java.j2", "DumpFunctions.java",
                                      tracked_dll_proj_decomp_output_path_format)

                            ghidra_cmd_binary_path = os.path.abspath(tracked_dll_version_file_path)
                            ghidra_cmd_script_dir = os.path.abspath(os.getcwd())

                            command = "{} {} {} -import {} -scriptPath {} -postScript DumpFunctions.java {}".format(
                                GHIDRA_HEADLESS, tracked_dll_proj_folder, tracked_dll_proj_name, ghidra_cmd_binary_path,
                                ghidra_cmd_script_dir, tracked_dll_proj_output_json)
                            ...
                            if (len(files.keys())) > 0:
                                ...
                                missing_version_windows_versions = missing_version['windowsVersions']

                                post_dict = {
                                    "name": tracked_dll_name,
                                    "description": missing_version['fileInfo']['description'],
                                    "virtual_size": missing_version['fileInfo']['virtualSize'],
                                    "size": missing_version['fileInfo']['size'],
                                    "sha256": missing_version['fileInfo']['sha256'],
                                    "version": missing_version['fileInfo']['version'],
                                    "signing_date": "1970-01-01 00:00:00" if (missing_version['fileInfo']['signingStatus'].lower() in ["unsigned", "unknown"] or "not trusted" in missing_version['fileInfo']['signingStatus'].lower()) else missing_version['fileInfo']['signingDate'][0],
                                    "functions": {},
                                    "update_info": missing_version_windows_versions,
                                }

                                # get our exports now so we dont have to do it in the loop...
                                tracked_dll_function_exports = get_dll_exports(tracked_dll_version_file_path)

                                for function_name in files['ghidra' + missing_version_hash]:
                                    # some dlls are too big to sensibly decompile in full
                                    if tracked_dll_name in EXPORT_ONLY_DLLS:
                                        if not function_name in tracked_dll_function_exports:
                                            continue
                                    
                                    function_decomp_path = tracked_dll_proj_folder + os.sep + tracked_dll_proj_name + "__" + function_name + ".txt"
                                ...
                                credentials = f"{BACKEND_USERNAME}:{BACKEND_PASSWORD}"
                                ...
                                response = requests.post(BASE_URL+"dlls", data=json.dumps(post_dict), headers=headers)
                                ...
                                zip_files(WORKING_DIR_PATH + tracked_dll_proj_name, "archive", missing_version['fileInfo']['sha256'])
        ...
        remove_file_if_exists(WORKING_DIR_PATH + "{}.txt".format(tracked_dll_name))
        remove_file_if_exists(WORKING_DIR_PATH + "{}.json.gz".format(tracked_dll_name))
