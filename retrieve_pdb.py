...
def download_pdb(download_dir, pdbname, signature):
    download_url = "https://msdl.microsoft.com/download/symbols/{}/{}/{}".format(
        pdbname, signature.upper(), pdbname
    )
    ...
    if r.status_code == 200:
        target_file = download_dir + os.path.sep + signature
        with progress.Progress() as p:
            progress_bar, csize = (
                p.add_task(
                    "[cyan]Downloading %s" % pdbname,
...
def get_pe_debug_infos(pathtopefile):
    ...
    raw_debug_data = [
        e
        for e in p.parse_debug_directory(
            pedata["IMAGE_DIRECTORY_ENTRY_DEBUG"].VirtualAddress,
            pedata["IMAGE_DIRECTORY_ENTRY_DEBUG"].Size,
        )
